...
namespace Fit4Job.DTOs.CompanyProfileDTOs
{
    public class EditCompanyProfileDTO
    {
        ...
        public void UpdateEntity(CompanyProfile companyProfile)
        {
            companyProfile.UserId = UserId;
            companyProfile.CompanyName = CompanyName;
            companyProfile.CompanyDescription = CompanyDescription;
            companyProfile.LinkedinUrl = LinkedinUrl;
            companyProfile.WebsiteUrl = WebsiteUrl;
            companyProfile.Industry = Industry;
            companyProfile.CompanySize = CompanySize;
            companyProfile.FoundingYear = FoundingYear;

            if (Status.HasValue)
            {
                companyProfile.Status = Status.Value;
            }
