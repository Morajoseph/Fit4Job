...
namespace Fit4Job.Repositories.Implementations
{
    public class CompanyTaskRepository : GenericRepository<CompanyTask>, ICompanyTaskRepository
    {
        ...
        {
            return await _context.CompanyTasks.Where(t => t.CompanyId == companyId)
        ...
        {
            return await _context.CompanyTasks.Where(t => t.IsActive).OrderByDescending(t => t.CreatedAt).ToListAsync();
        }

        public async Task<IEnumerable<CompanyTask>> GetTasksByDeadlineAsync(DateTime deadline)
        {
            return await _context.CompanyTasks.Where(t => t.Deadline.Date <= deadline.Date).OrderBy(t => t.Deadline).ToListAsync();
        }
    }
