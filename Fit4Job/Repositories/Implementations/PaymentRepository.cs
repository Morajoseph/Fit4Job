...
namespace Fit4Job.Repositories.Implementations
{
    public class PaymentRepository : GenericRepository<Payment>, IPaymentRepository
    {
        ...
        {
            return await _context.Payments.Where(p => p.PaymentStatus == status).OrderByDescending(p => p.CreatedAt).ToListAsync();
        }
        ...
        {
            return await _context.Payments.Where(p => p.PaymentStatus == PaymentStatus.completed).SumAsync(p => p.Amount);
        }
    }
}
